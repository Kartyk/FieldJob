app.controller("debriefController", function ($translate, $scope, $state, $rootScope, $window, $timeout, $filter, $http, $q, cloudService, $mdDialog, valueService, localService, Upload, constantService, $anchorScroll, $location) {
    ...
    $scope.expenseArray = [];
    $scope.expenseArraySummary = [];

    $scope.materialArray = [];
    $scope.materialArraySummary = [];

    $scope.notesArray = [];
    $scope.notesArraySummary = [];
    ...
    $scope.initializeDebrief = function () {
        ...
        $scope.notesArraySummary = valueService.getNote();
    ...
    $scope.setDefaultValues = function () {
        ...
        if ($scope.timeArraySummary != null && $scope.timeArraySummary.length > 0) {
        ...
        if ($scope.expenseArraySummary != null && $scope.expenseArraySummary.length > 0) {

            angular.forEach($scope.expenseArraySummary, function (item) {

                setExpenseValues(item);
            });
        }

        if ($scope.materialArraySummary != null && $scope.materialArraySummary.length > 0) {

            angular.forEach($scope.materialArraySummary, function (item) {

                setMaterialValues(item);
            });
        }

        if ($scope.notesArraySummary != null && $scope.notesArraySummary.length > 0) {

            angular.forEach($scope.notesArraySummary, function (item) {
    ...
    function setExpenseValues(item) {

        if (item.Date != undefined && item.Date != "")
            item.Date = new Date(item.Date);

        item.expenseDefault = expenseDefault;
    ...
    function setMaterialValues(item) {

        item.materialDefault = materialDefault;
        ...
        if (serialNo != undefined && serialNo.length > 0) {
            ...
            angular.forEach(serialNo, function (serialNumber) {

                var serialTypeObject = {};

                serialTypeObject.in = "";
                serialTypeObject.out = "";
                serialTypeObject.number = serialNumber;
        ...
        if (serialIn != undefined && serialIn.length > 0 && serialOut != undefined && serialOut.length > 0) {
            ...
            angular.forEach(serialIn, function (serialIn) {

                var serialTypeObject = {};

                serialTypeObject.in = serialIn;
    ...
    function setNotesValues(item) {

        if (item.Date != undefined && item.Date != "")
            item.Date = new Date(item.Date);

        item.noteDefault = noteDefault;
    ...
    function setDefaultExpenseObject() {
        ...
        $scope.expenseArray.push(expenseObject);

        if (expenseDefault.chargeMethod.values != undefined && expenseDefault.chargeMethod.values.length > 0) {

            angular.forEach(expenseDefault.chargeMethod.values, function (key, value) {

                if (key.Value == $scope.taskObject.Expense_Method) {

                    expenseObject.Charge_Method = key;
                    expenseObject.Charge_Method_Id = key.ID;
                }
            });
        }
    }

    function setDefaultMaterialObject() {
        ...
        $scope.materialArray.push(materialObject);

        if (materialDefault.chargeType.values != undefined && materialDefault.chargeType.values.length > 0) {

            angular.forEach(materialDefault.chargeType.values, function (key, value) {

                if (key.Value == $scope.taskObject.Material_Method) {

                    materialObject.Charge_Type = key;
                    materialObject.Charge_Type_Id = key.ID;
                }
            });
        }
    }

    function setDefaultNoteObject() {
            ...
            case "Expenses":
                ...
                if ($scope.isEditExpense == "0") {
                    ...
                    angular.forEach($scope.expenseArraySummary, function (response) {
                        ...
                        response.Expense_Id = $scope.taskId + "" + i;
            ...
            case "Material":
                ...
                if ($scope.isEditMaterial == "0") {
                    ...
                    angular.forEach($scope.materialArraySummary, function (response) {
                        ...
                        response.Material_Id = $scope.taskId + "" + i;
            ...
            case "Notes":
                ...
                if ($scope.isEditNote == "0") {
                    ...
                    angular.forEach($scope.notesArraySummary, function (response) {
                        ...
                        response.Notes_Id = $scope.taskId + "" + i;
    ...
    $scope.tabChange = function (stage) {
        ...
        if (stage.title == "Summary") {
            ...
            if ($scope.expenseArraySummary != undefined) {

                angular.forEach($scope.expenseArraySummary, function (key, value) {
            ...
            if ($scope.notesArraySummary != undefined) {
            ...
            if ($scope.timeArraySummary != undefined && $scope.timeArraySummary.length > 0) {
                ...
                angular.forEach($scope.timeArraySummary, function (key, value) {
    ...
    function updateFSRStatus(statusData) {

        var promiseArray = [];

        var attachmentJSONData = [];

        var deferEng = $q.defer();

        promiseArray.push(deferEng.promise);

        var deferCh = $q.defer();

        promiseArray.push(deferCh.promise);

        window.requestFileSystem(LocalFileSystem.PERSISTENT, 0, function (fs) {

            fs.root.getFile("Report_" + $scope.summary.taskObject.Task_Number + "_en.pdf", {
                create: true,
                exclusive: false
            }, function (fileEntry) {
                ...
                fileEntry.file(function (file) {
                    ...
                    reader.onloadend = function () {
                        ...
                        attachmentJSONData.push(reportObjectEng);
                    };
                ...
                fs.root.getFile("Report_" + $scope.summary.taskObject.Task_Number + "_ch.pdf", {
                    ...
                    fileEntry.file(function (file) {
                        ...
                        reader.onloadend = function () {
                            ...
                            $scope.chBase64 = this.result.split(",")[1];
    ...
    $(function () {
        ...
        $scope.showPopup = function (isemail = false) {

            $mdDialog.show({
                controller: PDFDialogController,
                templateUrl: "app/views/pdfDialog.html",
        ...
        $scope.openPdf = function (filename) {
            ...
            valueService.openFile(filePath + filename, "application/pdf", function () {
                ...
                promise.then(function () {
                    ...
                    valueService.openFile(filePath + "Report_" + $scope.summary.taskObject.Task_Number + "_en.pdf", "application/pdf");
                });
            });
        };
    ...
    $scope.uploadImage = function (file) {
        ...
        if (file != null) {
            ...
            Upload.base64DataUrl(file).then(function (urls) {
                ...
                fileObject.data = urls;

                fileObject.contentType = fileObject.data.split(",")[0].split(";")[0].split(":")[1];

                fileObject.base64 = fileObject.data.split(",")[1];

                $scope.image.push(fileObject);
    ...
    $scope.sendMail = function () {
        ...
        var compatibleAttachment, base64parts;
        ...
        if (valueService.getTask().Country == "People's Republic of China" || valueService.getTask().Country.toLowerCase() == "china") {
            ...
            if ($scope.reportBase64 != "" && $scope.reportBase64 != undefined) {
                ...
                openMailClent($scope.reportBase64, "Report_" + $scope.summary.taskObject.Task_Number + "_en.pdf")
                ...
                promise.then(function () {
                    ...
                    openMailClent($scope.value, "Report_" + $scope.summary.taskObject.Task_Number + "_en.pdf")
                });
            }
        }
    };

    function openMailClent(base64, filename) {
    ...
    function checkPdfHeight(doc1, ycord, height, yField, rectWidth, drawRect, rectHght) {
    ...
    function generatePDF(lang = "", isOptionsGiven = false) {
        ...
        setTimeout(function () {
            ...
            if (valueService.getTask().Country == "People's Republic of China" || valueService.getTask().Country.toLowerCase() == "china") {
                ...
                promiseArray.push(new Promise(function (resolve) { $scope.generateEnglishPDF(true, resolve); }));
                Promise.all(promiseArray).then(function (response) {
                    var englishDocRsponse = response[0];
                    promiseArray = [];
                    promiseArray.push(new Promise(function (resolve) { generateChinnesePDF(resolve); }));
                    if (promiseArray != undefined && promiseArray.length > 0) {

                        Promise.all(promiseArray).then(function (response) {
                            changeLanguage($scope.languageUsed);
                            if ($rootScope.local) {

                                var englishDoc = englishDocRsponse;

                                var chineeseDoc;

                                if (response.length == 1)
                                    chineeseDoc = response[0];
                                ...
                                fsrPromiseArray.push(new Promise(function (resolve) {
                                    ...
                                    $scope.value = englishDoc.output("datauri");
                                ...
                                if (chineeseDoc != undefined) {

                                    fsrPromiseArray.push(new Promise(function (resolve) {

                                        $rootScope.reposrtpath = filePath + "Report_" + $scope.summary.taskObject.Task_Number + "_ch.pdf";

                                        console.log("FILE PATH  " + filePath);

                                        $scope.value = chineeseDoc.output("datauri");

                                        valueService.saveBase64File(filePath, "Report_" + $scope.summary.taskObject.Task_Number + "_ch.pdf", $scope.value, "application/pdf", resolve());
                                    }));
                                }
        ...
        }.bind(lang, isOptionsGiven), 1000)

        return defer.promise
    }

    function generateChinnesePDF(resolve) {
        ...
        setTimeout(function () {
            ...
            {
                changeLanguage("ch", function () { chineesePdf(resolve) })
            }
            else {
                chineesePdf(resolve)
            }
    ...
    function chineesePdf(resolve)
    {
        ...
        Promise.all([new Promise(function (resolve) {
            ...
            if (canvas.getContext) {
                ...
                if (name.length == 1)
                    ctx.fillText(name[0], 30, 132);
                else {
                    isCustBig = true;
                    for (c = 0; c < name.length; c++) {
                        ctx.fillText(name[c], 30, 132 + c * 13);
                    }
                }
                ...
                ctx.fillStyle = "#000";
                ctx.strokeRect(20, 100, 1090, ibyvalue - 60);




                var i = 0, xNotesField = 20, yNotesField = ibyvalue + 55, rectNotesWidth = 660,
    ...
    function englishCanvasPdf(resolve) {
        var canvas = document.getElementById('englishcanvas');
        ...
        Promise.all([new Promise(function (resolve) {
            ...
            if (canvas.getContext) {
                ...
                ctx.fillText('No. 1277Xin Jingqiao Rd. Pudong,Shanghai, 201206', 720, 24);
                ...
                ctx.fillText($filter('translate')('Start Time'), timeWidth * columno++, yTimeFieldName);
                ctx.fillText($filter('translate')('End Time'), timeWidth * columno++, yTimeFieldName);
                ctx.fillText($filter('translate')('Duration'), timeWidth * columno++, yTimeFieldName);

                ctx.fillText($filter('translate')('Item'), timeWidth * columno++, yTimeFieldName);
                ...
                while (l < $scope.summary.materialArray.length) {
                    ...
                    if ($scope.summary.materialArray[l - 1].serialIn[n] != "") {
                        ...
                        while (n < $scope.summary.materialArray[l - 1].serialIn.length) {
                            ...
                            angular.forEach(splitserialin, function (key) {
                                ...
                                ctx.fillText(key, 400, yMaterialSerialIn);
                                yMaterialSerialIn += 15;
                            })
                            // ctx.fillText($scope.summary.materialArray[l - 1].serialIn[n++], 350, yMaterialSerialIn);
                        }
                    ...
                    if ($scope.summary.materialArray[l - 1].serialOut[o] != "") {
                        ...
                        while (o < $scope.summary.materialArray[l - 1].serialOut.length) {
                            ...
                            angular.forEach(splitserialout, function (key) {
                                ...
                                ctx.fillText(key, 550, yMaterialSerialOut);
                                yMaterialSerialOut += 15;
                            })
                            // ctx.fillText($scope.summary.materialArray[l - 1].serialOut[o++], 450, yMaterialSerialOut);
                        }
                    ...
                    if ($scope.summary.materialArray[l - 1].Description) {
                        ...
                        angular.forEach(splitTitle, function (key) {
                            ...
                            ctx.fillText(key, 800, yMaterialFieldValue);
                            yMaterialFieldValue = yMaterialFieldValue + 15;
                        });
    ...
    function englishPdf(isChineese, resolve, defer)
    {
        ...
        $q.all(promiseArray).then(function (response) {
            ...
            if ($scope.summary.taskObject.Customer_Name.match(/[\u3400-\u9FBF]/)) {
                ...
                // doc1.addImage(response[0], 'JPEG', 25, 110);
            }
            promiseArray = [];
            promiseArray.push(new Promise(function (resolve) {
                ...
                new Promise(function (resolve) { doc1.addImage(imgData, 'JPEG', 30, 15, 100, 40, 'emelogo', 'FAST'); resolve(); });
                ...
                if (isCustBig)
                    custBigYvalue = 10;
                doc1.text(25, 125 + custBigYvalue, $filter('translate')('Service Request'))
                doc1.setFontSize(22)
                doc1.setFontType('normal')
                if ($scope.summary.taskObject.Service_Request)
                    doc1.text(25, 135 + custBigYvalue, $scope.summary.taskObject.Service_Request)
                doc1.setFontSize(22)
                doc1.setFontType('bold')
                doc1.text(180, 125 + custBigYvalue, $filter('translate')('Field Job Request'))
                doc1.setFontSize(22)
                doc1.setFontType('normal')
                if ($scope.summary.taskObject.Task_Number.toString())
                    doc1.text(180, 135 + custBigYvalue, $scope.summary.taskObject.Task_Number.toString())
                doc1.setFontSize(22)
                doc1.setFontType('bold')
                doc1.text(345, 125 + custBigYvalue, $filter('translate')('Job Description'))
                doc1.setFontSize(22)
                doc1.setFontType('normal')
                ...
                while (i < $scope.summary.notesArray.length) {
                    ...
                    doc1.setFontSize(22)
                    doc1.setFontType('normal')
                    var isAdded = checkPdfHeight(doc1, yNotesField1_val, pageHeight, yNotesField, rectNotesWidth);
                    if (isAdded) {
                        yNotesField1_val = 10;
                        yNotesField = -10;
                    }
                    ...
                    if ($scope.summary.notesArray[i - 1].Notes) {
                        ...
                        angular.forEach(splitTitle, function (key) {
                            ...
                            doc1.text(xNotesField2, yNotesField1_val, key)
                            yNotesField1_val += 10;
                            yNotesField1 = yNotesField1_val;
                        })
                ...
                doc1.setFontSize(22)
                doc1.setFontType('bold')
                var isAdded = checkPdfHeight(doc1, yAttachField, pageHeight, yAttachField + 5, rectAttachWidth, false);
                if (isAdded) {
                    yAttachField = 10;

                }
                doc1.text(xAttachField, yAttachField, $filter('translate')('Attachments'))
                var isAdded = checkPdfHeight(doc1, yAttachField + 5 + 50, pageHeight, yAttachField + 5, rectAttachWidth, true, rectAttachHeight);
                if (isAdded) {
                    yAttachField = -5;
                ...
                doc1.setFontSize(22)
                doc1.setFontType('bold')
                var isAdded = checkPdfHeight(doc1, yTimeField + 40, pageHeight, yTimeField + 40, rectTimeWidth, false);
                if (isAdded) {
                    yTimeFieldName = 30;
                    yTimeField = 10;
                }
                ...
                doc1.setFontSize(22)
                doc1.setFontType('bold')
                var isAdded = checkPdfHeight(doc1, yTimeFieldName, pageHeight, yTimeField, rectTimeWidth);
                if (isAdded) {
                    yTimeFieldName = 30;
                    yTimeField = 5;
                }
                ...
                while (j < $scope.summary.timeArray.length) {
                    ...
                    doc1.setFontSize(22)
                    doc1.setFontType('normal')
                    var isAdded = checkPdfHeight(doc1, yTimeFieldValue, pageHeight, yTimeField, rectTimeWidth);
                    if (isAdded) {
                        yTimeFieldValue = 10;
                        yTimeField = -10;
                        indexTime = 0;
                ...
                //    var isAdded = checkPdfHeight(doc1,yExpenseFieldValue, pageHeight, yExpenseField, rectExpenseWidth);
                //    if (isAdded) {
                ...
                var rectMaterialWidth = 660, rectMaterialHeight = 25 * $scope.summary.materialArray.length,
                    yMaterialFieldName = yMaterialField + 25, yMaterialFieldValue;
                doc1.setFontSize(22)
                doc1.setFontType('bold')
                var isAdded = checkPdfHeight(doc1, yMaterialField + 5, pageHeight, yMaterialField, rectMaterialWidth, false);
                if (isAdded) {
                ...
                doc1.setFontSize(22)
                doc1.setFontType('bold')
                var isAdded = checkPdfHeight(doc1, yMaterialFieldName, pageHeight, yMaterialField, rectMaterialWidth);
                if (isAdded) {
                ...
                while (l < $scope.summary.materialArray.length) {
                    ...
                    doc1.setFontSize(22)
                    doc1.setFontType('normal')
                    var isAdded = checkPdfHeight(doc1, yMaterialFieldValue, pageHeight, yMaterialField, rectMaterialWidth);
                    if (isAdded) {
                ...
                doc1.setFontSize(22)
                doc1.setFontType('bold')
                var isAdded = checkPdfHeight(doc1, ySignField + 5, pageHeight, ySignField, rectSignWidth, false);
                if (isAdded) {
                    ySignField = 10;
                }
                doc1.text(xSignField, ySignField + 5, $filter('translate')('Signature'))
                var isAdded = checkPdfHeight(doc1, ySignField + 25, pageHeight, ySignField, rectSignWidth);
                if (isAdded) {
                    ySignField = 0;
                }
                ...
                var isAdded = checkPdfHeight(doc1, ySignField + 35, pageHeight, ySignField, rectSignWidth);
                if (isAdded) {
                    ySignField = 0;
                }
