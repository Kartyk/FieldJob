app.controller("debriefController", function ($translate, $scope, $state, $rootScope, $window, $timeout, $filter, $http, $q, cloudService, $mdDialog, valueService, localService, Upload, constantService, $anchorScroll, $location) {
    ...
    $scope.customerSubmit = function () {
        ...
        //            promise = generatePDF(selected.ID, true);
        //        else
        //            promise = generatePDF();
        //        submit(promise);
        //    }, function () {
    ...
    $(function () {
        ...
        setTimeout(function () {
            ...
            $("#cmd").click(function () {
                    ...
                    {
                        $scope.openPdf( "Report_" + $scope.summary.taskObject.Task_Number + ".pdf");
                    }
    ...
    $scope.sendMail = function () {
        ...
        {
            if ($scope.reportBase64 != "" && $scope.reportBase64 != undefined) {
                openMailClent($scope.reportBase64, "Report_" + $scope.summary.taskObject.Task_Number + ".pdf")
            } else {

                var promise = generatePDF();

                promise.then(function () {
                    openMailClent($scope.value,"Report_" + $scope.summary.taskObject.Task_Number + ".pdf")                   
                });
            }
        }
    };
    ...
    function generatePDF(lang = "", isOptionsGiven = false) {
        ...
        setTimeout(function () {
            ...
            var promiseArray = [];
            if (valueService.getTask().Country == "People's Republic of China" || valueService.getTask().Country.toLowerCase() == "china") {
                //if (isOptionsGiven && ( lang == 'ch' )) {
                //    promise = generateChinnesePDF();
                //}
                //else if (isOptionsGiven && lang=='en')
                //{
                //    promiseArray.push($scope.generateEnglishPDF(true));
                //}
                //else
                //{
                //    promise = generateChinnesePDF();
                ...
                Promise.all(promiseArray).then(function (response) {
                    if ($rootScope.local) {
                        ...
                        fsrPromiseArray.push(new Promise(function (resolve) {
                            ...
                            valueService.saveBase64File(filePath, "Report_" + $scope.summary.taskObject.Task_Number + ".pdf", $scope.value, "application/pdf", resolve());
                        }))
